...
export function EntitlementStore(props: EntitlementStoreProps) {
  ...
  const dismissNotice = useMutation({
    mutationFn: async () => {
      ...
      if (!response.success) {
        throw new Error(
          response.message || t("Failed to save your preference"),
        );
      }
    },
    onSuccess: async () => {
  ...
  const purchase = useMutation({
    mutationFn: async (input: {
      ...
      const requestedGiftCents = Math.round(
        Number(giftAmountYuan || "0") * 100,
      );
      ...
      if (orderResponse.data.cash_payable_cents === 0) {
        return orderResponse.data.gift_discount_cents > 0 ? "gift" : "free";
      }
