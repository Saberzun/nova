...
func TestComputeTieredQuota_ClampOnOverflow(t *testing.T) {
	// exprOutput = p * 1e12 produces a value beyond the int64 ledger.
	...
	assert.Equal(t, math.MaxInt64, result.ActualQuotaAfterGroup, "oversized quota must clamp to int64 max, never wrap negative")
	...
	assert.Equal(t, math.MaxInt64, result.Clamp.Clamped)
}
